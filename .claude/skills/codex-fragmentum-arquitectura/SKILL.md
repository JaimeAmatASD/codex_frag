...
### El contenido es enchufable (ADR-007)

Mundos, paquetes de lore y seres individuales son cartuchos: se enchufan y desenchufan de la máquina. Enchufar es COPIAR (fork que diverge), nunca linkear ni sincronizar: no existe "el mismo" ser vivo en dos mundos, existen descendientes de un mismo paquete, ambos legítimos. El paquete de un ser separa lo que viaja (alma, corpus, hitos, voz), lo que se recalcula (embeddings), lo que es del mundo (posición, relaciones, información poseída) y lo que es de las reglas (hoja mecánica, se regenera). Un ser puede entrar a un mundo primero como leyenda — solo su fama circulando por el grafo — y encarnar después (gradiente sobre ADR-006). En el código presente esto solo exige: ID local estable más campo origen en toda entidad, carpeta autocontenida por ser, y hoja mecánica en archivo separado. PROHIBIDO construir tooling de import/export antes de que existan dos mundos reales que quieran intercambiar algo.
