...
Restricción del ADR-007 (contenido enchufable) que aplica desde este paso: toda entidad nueva lleva ID local estable y limpio más un campo origen (string, default el id del mundo que la contiene). Nada de prefijos compuestos en los IDs internos; el compuesto origen:id se materializa solo cuando dos mundos se toquen, en el futuro. Agregale también el campo origen a los seres existentes (un campo más en su JSON, default "prueba").
